...
**Live**: `http://marketpilot-prod-alb-1177715901.us-east-1.elb.amazonaws.com` — full market-data → analysis → signal → risk → paper-trade pipeline verified against this URL. Phase 9.6 re-verified the deployment end to end (zero Terraform drift, security groups/IAM/secrets re-audited live, all CloudWatch alarms OK); Phase 9.7 added a CI Docker-build validation job, re-confirmed the same three blockers with stronger evidence (an SSH-key auth test, not just "no `gh` CLI"), and corrected the Redis transit-encryption risk assessment (an in-place update, not destructive — verified via a local-only `terraform plan` probe, never applied). Three genuine environmental blockers remain, stopped at rather than faked: no GitHub remote/auth (CI/CD implemented, never executed), no domain (HTTPS ready, not active), no real Claude API key (wiring verified, AI Analyst inactive) — plus a fourth, `REDIS_TLS_MIGRATION_REQUIRED`, now correctly identified as application-side. See [docs/aws-deployment.md](docs/aws-deployment.md) §0/§10/§11 for the exact status of each.
