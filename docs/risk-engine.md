...
**Concurrency (Phase 9.5 hardening)**: `RiskEvaluation.signal_id` deliberately has no unique constraint (an audit-history table), so nothing at the database level backstopped "one evaluation per signal, ever" — two genuinely simultaneous `POST /risk/evaluate/{id}` requests could both pass the CANDIDATE check before either committed, producing two evaluations and a silently double-written status (reproduced by `tests/test_risk_concurrency.py`). The status check is now performed twice: a provisional, unlocked check that fails fast, then — after `get_quote()`'s internal commit, which would otherwise release the lock — an authoritative re-check under `SELECT ... FOR UPDATE` (with `populate_existing()`, since SQLAlchemy's identity map would otherwise hand back the stale pre-lock object) immediately before the evaluation is written. The losing concurrent request now blocks on the row lock, re-reads the real (no-longer-CANDIDATE) status, and gets the same clean `409` a sequential second call always got. Different signals evaluate concurrently without serializing against each other.
