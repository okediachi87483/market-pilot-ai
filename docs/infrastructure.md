...
- **Redis has no transit encryption** (§6) — a deliberate, bounded tradeoff (Redis is cache/pub-sub only, never authoritative, per `docs/architecture.md`). Phase 9.7 correction: the AWS-side change turns out to be *low-risk* — probed live via a local-only `terraform plan` (edited, planned, reverted, never applied) and `transit_encryption_enabled = true` reports as an **in-place update** (`0 to add, 1 to change, 0 to destroy`) with the pinned provider, not a destructive replacement as earlier phases assumed. Phase 9.8: the application-side blocker is now closed — `Settings.redis_tls_enabled` (default `false`) switches `Settings.redis_url` to `rediss://`, and `infrastructure/terraform/variables.tf`'s `redis_transit_encryption_enabled` (default `false`) drives both the ElastiCache flag and the ECS `REDIS_TLS_ENABLED` env var from one switch, so the two sides can no longer drift apart. Both still default off — the platform still runs plaintext `redis://` in production; flipping the variable is a live, two-sided production change requiring a manual `force-new-deployment` (no GitHub CI/CD path exists yet to do this through the normal pipeline), so it was prepared and documented (`docs/aws-deployment.md` §"Redis TLS rollout") but not applied, pending explicit human approval (`REDIS_TLS_MIGRATION_REQUIRED`).
