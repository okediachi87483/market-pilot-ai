...
| `risk-engine` | Deterministic evaluation of `RiskPolicy` against current portfolio state and a proposed order. Approves or rejects, and computes position size/stop-loss/take-profit itself. Contains no LLM calls and no configuration the AI layer can write to. | `paper-trading`, `shared` |
| `paper-trading` | Simulated brokerage: order lifecycle, fills, fees, position tracking, *and* the portfolio-state computation (equity, exposure, P/L, drawdown) the risk engine reads. Only accepts orders that already carry a risk-engine approval. | `shared` |
| `alerts` | Evaluates alert conditions (signal-based, risk-threshold-based, profit-protection-based — see [profit-protection.md](profit-protection.md)) and writes `Alerts`. Never executes anything. | `paper-trading`, `risk-engine`, `shared` |
