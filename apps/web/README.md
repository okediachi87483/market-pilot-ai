...
└── market/     MarketExplorer, PriceChart, AnalysisPanel, MarketStateVisualization,
                SignalCenter, SignalCard, RiskCenter, WatchlistPreview, SignalPreview,
                RiskPreview — real API data (Phase 3/4/5/6); portfolio/alerts remain
                mock placeholders until their owning phases land
lib/            nav definitions, API fetch wrapper (api.ts), market data client
                ...
                engine client (signals.ts), risk engine client (risk.ts)
styles/         design tokens (docs/ui-design-system.md)
```

`/markets`, `/signals`, `/risk`, and the dashboard are wired to the real backend: `GET /api/v1/assets`, `/market/{symbol}`, `/analysis/{symbol}`, `/analysis/{symbol}/indicators`, `POST /signals/evaluate/{symbol}`, `GET /risk`, `POST /risk/evaluate/{signal_id}` — the underlying market data itself is still mock end to end (docs/market-data.md, docs/technical-analysis.md, docs/signal-engine.md), always labeled `SOURCE: MOCK`. The dashboard's signature gauge is a real "Market State" visualization driven by the detected regime (descriptive); the Signal Center (`/signals`) is the separate, prescriptive-within-a-named-strategy view of what `trend_momentum_v1` currently suggests — see docs/signal-engine.md §10 for why these stay visually and conceptually distinct. The Risk Center (`/risk`) is the deterministic safety boundary's own view — portfolio state, active policy limits, and recent approve/reject decisions (docs/risk-engine.md §12); the Signal Center shows each signal's live progress through that same review.
